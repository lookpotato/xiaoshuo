...
单书配置 `report_contract: 2` 时，所选模块 `guide.steps` 为操作方法，`guide.checks` 为逐项审稿题。
每个模块报告必须增加 `checks`，键名对应题目 id，值为：

```json
{
  "orientation": {
    "verdict": "met",
    "answer": "仅陈述引用实际能支持的理解，不往原句里补设定",
    "support": "说明原句中的哪些词直接支撑回答；额外推测需另列",
    "evidence": [{"chapter": 89, "quote": "逐字存在的正文原句", "narrative_sha256": "该章节正文哈希"}]
  }
}
```

`verdict` 为 met/gap/not_applicable。met 必须提供证据与支撑说明；gap 必须写 `fix`，指定修订位置和动作，模块 status 必须为 needs_revision。not_applicable 必须具体说明原因，不可因难回答而跳过。
世界认知每条增加 `support_kind`（explicit/inferred/missing）和 `required_now`（布尔）。推断不能登记 known；本章必需的认知若仍为 unknown 或 hinted，模块必须标记 needs_revision。
程序仍不能自动判断“这句证据真的足以支撑解释”。审稿者应主动寻找不支持结论的地方，真实读者验证仍独立进行。
模块中的前后改写均是示例，不是经过本书设定核对的替换正文。
协议1继续兼容；升级协议2会重新检查当前生效范围的报告。不要在运行批次中升级。
