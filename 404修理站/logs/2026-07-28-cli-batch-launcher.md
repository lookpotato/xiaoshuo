...
- 实际命令行批次验证发现，VS Code 启动的 `codex exec` 子进程无法取得 Codex 桌面
  已登录浏览器，返回 `No browser is available`，因此两次测试 job 均为 0 个成功槽位。
- 固定命令改为只创建持久 job 队列；Codex 桌面自动任务负责领取并执行浏览器发布。
- 工作器使用 `job-next` 原子领取 job 和全局锁，避免终端子进程误报上传成功。
