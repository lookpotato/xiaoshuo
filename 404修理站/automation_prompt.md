...
2. 读取根目录 `shared/writing_playbook.md`、`shared\quality_scorecard.md`、
   `shared/image_workflow.md`、`shared/reader_gate.md` 和 `shared\learning_log.md`，再读取本书全部核心设定、`chapter_state.json`、
...
4. 若没有待上传章节，则规划并生成 `next_chapter_number`，进行连续性、章节结构、文风、
   平台风险和 100 分质量评分；先按 `shared/writing_playbook.md` 确定本章主情绪、一个可围观的瓜、
   两个轻松/痛快互动节拍与一个评论区按钮，再设计必要剧情。第65章起情绪价值是首要硬门槛：
   若正文主要体验仍是核验装置、比对档案或公布推理答案，即使逻辑无误也只能留草稿重写。
   每批开始前必须联网观察近期公开热梗与评论表达，在运行日志记下1—3个准备采用的情绪结构；
   只提取结构并转译成维修站人物语言，不照搬完整热句。临时无法联网时沿用最近一次日志，不阻塞写作。
