...
4. 若没有待上传章节，则规划并生成 `next_chapter_number`，进行连续性、章节结构、文风、
   ...
   番茄上传回显。文件未落盘、损坏、画幅错误或目录校验失败时，必须关闭本次图片专用 Chrome，重新启动并重新生成，最多尝试两次；第二次仍失败后关闭浏览器，只保留草稿，不得归档。
   ...
   追读优先于机制完整：先写清本章具体人物想要什么、谁在阻拦、失败会让谁失去什么，
   再让规则在动作、误会和代价中显现；禁止把章节写成“发现机制→解释机制→寻找漏洞→公布新名词”的流水线。
   每场戏至少保留一个人物私心、隐瞒、误判或与主方案相冲的选择；删去设备名和规则名后，
   若人物之间没有值得关心的冲突，必须重写。
