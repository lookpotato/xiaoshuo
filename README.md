...
生图固定通过 `browser_image_worker.py` 使用独立的已登录 Chrome 网页完成，不消耗 Codex 图片生成额度，也不要求 API Key。成图直接下载回本书目录，再由 Codex 用 `view_image` 回看身份、标志特征、颜色材质、形状部件、设定冲突、文字水印和裁剪安全构图七项；任一项不符就修正提示词并通过网页定向重生。只有七项全通过，才允许在目录中记录 `verified`。程序还会复核文件头、SHA-256、分类目录、单图上限和正文引用；失败时只能保留草稿，不会改用 Codex 生图、归档章节或推进章节号。完整规范见 `shared/image_workflow.md`。
