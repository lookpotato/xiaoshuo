...
确认“用于本书”后，经验写入本书 `feedback_learning.json` 并同步到 `style_guide.md`；确认“教给作者”后，经验写入绑定作者档案并同步到该作者当前绑定作品的 `style_guide.md`。旧生成流程读取文风指南，V2 流程读取结构化知识库或作者档案，因此确认后的经验会参与后续生成与审稿。相同规则再次确认时只累加反馈证据，不重复堆叠规则。
