...
工作台“读者反馈”现在作为副作者审稿入口。绑定作者会先判断反馈并给出当前章候选修订；若意见可以跨句、跨场景复用，还会生成包含适用条件与误用边界的长期经验候选。当前章修订和长期学习分别确认，互不捆绑。

每条“作者判断记录”支持连续讨论。副作者可在原记录下纠正误判或继续追问，完整对话本地落盘；作者若改判，旧分析进入 `analysis_history/`，新分析与候选修订同步替换并在界面标明“已修正判断”。网络失败不会丢失副作者消息，可在同一记录中重试。

## 质量优先的多请求写作链

章节生产不再把全部判断压在一次 Codex 调用中。启用 `novel_pipeline.json` 后按“独立导演 → 作者写章 → 陌生中文对白试读 → 必要时专项返修并复读 → 独立文学终审 → 机械归档门禁”串行执行。中文对白试读只看到当前正文，不读取人设或作者意图；它不能用“符合人设”替现实口语问题免责。对白修订最多两轮，每轮修完都用新的独立上下文重新判断，仍不通过则保留现场文件并终止归档。

局部反馈默认只允许修改选中原文附近，不能借一句意见暗中重写或压缩整章。采用候选稿前会按本书配置检查实际章长并重算 Metadata 字数；正文一旦改变，旧读者验收会先备份并撤销，重新验收前不得发布。

确认“用于本书”后，经验写入本书 `feedback_learning.json` 并同步到 `style_guide.md`；确认“教给作者”后，经验写入绑定作者档案并同步到该作者当前绑定作品的 `style_guide.md`。当前唯一生产流程会同时读取绑定作者、本书结构化经验和文风指南，因此确认后的经验会参与后续生成、自动返修与审稿。相同规则再次确认时只累加反馈证据，不重复堆叠规则。

## 统一生成入口

工作台、`xiaoshuo.py` 和旧的 `novel_v2.py run` 现在都汇入 `xiaoshuo.py` → `xiaoshuo_on_demand.py` 这一条生产流程。它统一负责作者写作、能力装配、人物私线、读者门禁、自动返修、归档，以及按运行选项执行配图、番茄发布和 Git 同步。原 V2 目录只保留作者档案与内部上下文组件，不再维护第二套章节生成状态。

## 可按书装配的创作能力

能力区采用左侧列表与右侧详情，分为“怎么起作用”“怎样判断有用”“本书配置”。每项包含具体操作、两项审稿问题及改写示例，并明确区分程序检查与尚未验证的阅读效果。最近运行记录仅展示报告自述，不视为质量证明。
单书 `report_contract: 2` 开启逐项回答、证据支撑说明和修订动作检查；世界认知区分明示与推断。本次《404》从第89章使用协议2。模块效果仍需真实读者对照验证，单元测试通过只说明软件行为符合预期。

在工作台“小说设置 → 创作能力装配”中选择模块，分别设置关闭、观察或强制验收、启用章节和本书重点，然后保存。12 个内置能力包括世界观呈现、场景与行动、人物自主性、对白与潜台词、因果与选择、情绪铺垫与兑现、节奏与变化、伏笔与承诺兑现、风险与资源代价、关系演进、连载回读、真实读者反馈。

共享定义在 `shared/creative_modules.json`，可在“系统设置”编辑；单书配置在各书 `creative_modules.json`。新增模块只要填写完整能力定义即可进入各书选择列表，未配置模块默认关闭。当前《404修理站_重启版》从第 89 章强制启用世界观呈现，避开正在执行的第 69—88 章批次；其他能力供选择，其他书默认关闭。已有章节保持原有要求。

生成及自动修复会装配当前启用模块，写前保存 `module_plans/NNNN.json`，写后保存 `module_reports/NNNN.json`。世界观模块记录五个认知维度及跨章证据，每 5 章复盘；首次接入须回查旧正文建立认知基线，不自动假定读者已知道设定。机器验证正文/计划哈希和证据存在性，文学判断仍需 AI 审稿及真实读者反馈。观察模式允许文学问题，但不允许缺失或伪造报告；强制模式要求解决问题后才能继续归档流程和上传。

命令：`python creative_modules.py catalog` 查看能力库，`python creative_modules.py prompt <书籍目录>` 查看下一章装配，`python creative_modules.py validate <书籍目录>` 单独验收。管理器原有读者门禁已接入同一验收。完整协议与报告示例见 `shared/creative_modules_workflow.md`。
...
左侧“小说设置”管理每本作品自己的运行参数与创作模块，包括总提示词、基础配置、文风、专属叙事包、人物声音、故事总纲、人物设定和世界观；修改一本书不会覆盖其他书。“系统设置”管理全部作品共享的运行底座、模块注册表和共享提示模块，包括中国人物对白、叙事底座、独立人物引擎、多人物顺序线、质量评分与读者门禁。配置读写由独立的 `settings_service.py` 提供，接口只允许访问白名单文件，使用文件版本号防止并发覆盖，并在小说任务运行时自动切换为只读。

每本书的设置页还提供“人物独立故事线”：系统直接读取 `character_threads/四位章节号/序号-人物名.md`，按人物聚合成逐章时间轴，可查看和修订该人物当章的目标、误解、主动行动、代价与下一步。人物私线只参与本地连续性维护，不会作为小说章节上传；任务运行期间只读，保存时同样检查文件版本，防止覆盖刚生成的新状态。

设置接口为 `GET /api/settings?scope=book&book_id=<id>`、`GET /api/settings?scope=system` 和 `POST /api/settings`。每本新小说只需在 `manager_config.json` 注册目录并放入所需的单书模块文件，即可复用同一套前后端管理页面。
...
运行配置下方固定提供独立的“后端实时控制台”，无需打开弹窗。新任务启动或任务失败时会自动切换到对应运行记录并单独标出最终失败原因；也可从下拉框切换历史运行。控制台通过 `/api/run-log` 在运行中每 2 秒刷新，任务结束后日志仍会保留并可反复查看。上下文可选最新 100、300 或 500 行，后端硬上限为 500 行及 256 KB；接口同时过滤小说正文、隐藏常见凭据字段并截断异常超长单行。

本地章节写作采用自动修复闭环：若下一章的人物线、读者验收、正文哈希、因果证据或状态回写门禁失败，系统会回滚章节状态，把结构化错误交回 Codex 定点修复并重新校验，最多自动修复两次。上一轮留下的下一章问题也会在下次运行时直接进入修复流程；缺少项目文件、其他历史章节损坏等非本章问题仍会安全停止，避免 AI 扩大修改范围。

### 通用分阶段创作

所有注册作品共用 `novel_pipeline.json` 与 `novel_stage_pipeline.py`，公共流程不包含任何书名、人物名或题材专属规则。每章先在独立上下文中生成 `chapter_plans/NNNN.json`，明确人物诉求、中心选择、情绪推进、旧期待回应、新增悬念上限和不可逆结果；作者只依据本书资料与该合同写作。

网页“章节归档”中的删除按钮执行尾部回退：选择第 N 章会把第 N 章及其后所有本地正文、草稿、章节合同、读者验收、对白/文学审稿、人物私线和读者反馈一起移出活动项目，并把 `chapter_state.json` 回退到第 N 章。文件保存在项目的 `.chapter_trash/<批次>/` 中，不会立即永久擦除；已经上传到平台的章节拒绝在本地单独删除。删除期间如果存在后台任务，接口也会拒绝操作，避免生成与回退同时写文件。

正文完成后，系统另开一个看不到大纲、设定、作者档案和章节合同的上下文，只读取近期正文与当前章，生成 `literary_reviews/NNNN.json`。终审分别检查人物动机、情绪推进、现场对白、实质进展、承诺兑现和下一章吸引力。无大纲读者验收仍负责因果与信息完整，不能再单独宣布章节合格；文学终审未通过时同样回到自动返修，中心任务错误则退回章节合同。

新增小说时只需提供项目合同要求的作品文件并在 `manager_config.json` 注册，不需要修改公共程序或复制另一部小说的提示词。缺失的可选资料会被跳过，其他作品的资料不会进入当前写作上下文。
