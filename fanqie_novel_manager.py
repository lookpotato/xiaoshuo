...
FANQIE_FIXED_UPLOAD_STEPS = [
    ...
    "若本章有唯一配图，在页面底部“作者有话说”悬停“添加”，点击“添加图文”，再点击输入框左下角图片按钮。",
...
FANQIE_SUCCESS_CHECKS = [
    ...
    "章节本地有图时，“作者有话说”必须回显唯一图片；空白、重复图片或上传弹窗未确认都不得点击下一步。",
...
def cmd_session(data, args):
    ...
    payload = {
        ...
        "batch_workflow": [
            ...
            "使用图片专用已登录 Chrome 调用网页 GPT 逐张生成；每次生成或下载超时/失败后关闭本次图片专用 Chrome，重新启动并重新提交同一提示词，最多尝试两次；第二次仍失败后关闭浏览器并只保留草稿。下载后直接采用，不调用 view_image 或其他 Codex 视觉回看，也不因主观画面判断重生。只校验文件头、SHA-256、像素画幅、分类目录、正文引用与番茄上传回显。",
